...
url = "http://%s/org/pronto/pronto.nsf/(OpePRAtMbyID)?OpenView&Group=%s" % (server, group)
...
ISUG = ['some_name_from_the_team','some_other_name_from_the_team','third_name..']
new_prontos = []
Persons = {}
...
def get_pronto_ids(r):
    ...
    severity=re.findall(r'<td valign=top>(A|B|C)(?=\s*&nbsp|\s*<img src="Customer.gif")',r.content)
    ...
    if not content:print 'Could not fetch any prontos ';return 2
...
def get_pronto_page(pronto_id,server):
    ...
    try:
        ...
        #:After we have the id get login into the specific pronto and parse the Corrections for Responsible Person :#
            r = requests.get(vpn_url % (server,p_id),auth=(username,password))
            if r.reason == 'OK':
                pronto_title = re.search('name="PRMainTitle" type="hidden" value=.+PR .+?:(.+)>',r.content).group(1).strip('" ')
                pr_severity  = re.search('PRSeverity" type="hidden" value="(A|B|C) - \w+',r.content).group(1).strip()
             #   print 'Searching for correction task for %s ' % ('\n'+pronto_title)
                corrections = re.search('SDInformation.+value=(.+)>',r.content).group(1).split(',')
...
def parse_correction_tasks(correction_list,pronto_id,index,server):
    ...
    if correction_list:
        ...
        for i in correction_list:
            correction_ids.append(i[1:i.find('&amp')-2])#: -2 is for \xa characters :#
            try:       
                r = requests.get(finalvpn_url % (server,correction_ids[0],pronto_id),auth = (username,password),timeout=5)
            except requests.exceptions.Timeout:
                print 'Could not fetch main pronto page for %s ' % pronto_title
                raise
            resp_pers = re.search('MCResp.+value=(.+)>',r.content).group(1).strip('"')
            ...
            print 
            print '='*50
            break #: exit the loop as the correction tasks may be many,usually coordinated by the same person...#:
    else:
...
def keep_ISUG_prontos():
    ...
    ISUG_prontos = [pronto for pronto in new_prontos if re.search('ISUG',pronto,re.I)]
    return ISUG_prontos
