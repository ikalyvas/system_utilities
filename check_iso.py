...
def init_globals():
    ...
    try:
        ...
        pass
    url = 'http://hugin.dev.cic.org-rdnet.net/scit/html/projects/ISO_' + \
        branch + '_delivery_AB4/index.html'
    timer = 900  # 15minutes
...
def wait_until_completion(page):
    ...
    while (countdown != 0):
        ...
        page = get_page(url)
        # start gradually reducing by 15m the whole duration 2h...
        countdown -= timer
...
def send_description():
    ...
    for line in urllib2.urlopen(url):
        if 'Latest completed revision' in line and 'success' in line and commited in line:
            success_link = re.search(
                'href=("\/scit\/html\/ISO.+)>success', line).group(1).strip('"')
            success_link = hugin + success_link
            success_link = success_link.replace('.html', '_log')
            success_link = success_link + '/' + \
                re.search('AB4_(.+)_log', success_link).group(1) + \
                '_3:ISO_delivery_' + branch + '_AB4_s1'
...
def fail_and_exit():
    ...
    raise FatalError(
        'ISO should have been triggered with %s.Unknown fatal error' % commited)
